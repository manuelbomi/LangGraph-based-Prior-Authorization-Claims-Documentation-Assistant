...
Design: each prompt has a `name` (e.g. "extract_clinical_summary",
"evaluate_policy_criteria", "draft_pa_request") and one or more `version`s.
Exactly one version per name is flagged `is_active`; that is the version
`get_prompt()` returns. This lets you iterate on prompts (add a new
version, A/B it, roll back) without redeploying code -- you just insert a
new row and flip `is_active`.
